...
export const DappOSNetworkSwitcher = () => {
  const { t } = useTranslation()
  const { srcChainId: chainId, isWrongNetwork, isNotMatched } = useActiveChainId()
  ...
  return (
    <Box ref={cannotChangeNetwork ? targetRef : null} height="100%">
      ...
      <UserMenu
        ...
        avatarSrc={chains.find((i: any) => i.id === chainId)?.chainLogo ?? ''}
